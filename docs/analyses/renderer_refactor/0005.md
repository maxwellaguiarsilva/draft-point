## 0005: Detalhamento Algorítmico e Sincronização
	...
	auto elements( )
	{
		...
		return	iota( 0, static_cast< int >( m_container.size( ) ) )
			| transform( [ this ]( int index ) {
				//	derivação espacial baseada em 1
...
Implementação de `trace_line` como um gerador ( C++23 ).

```cpp
inline auto trace_line( pixel start, pixel end ) -> ::std::generator< pixel >
{
...
O novo loop de atualização utiliza `m_back_view.elements( )` e elimina qualquer cálculo de índice manual no buffer frontal.
