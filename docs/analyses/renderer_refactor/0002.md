## 0002: Arquitetura de Espaços e Superfícies
...
cell::point cell_position = to_cell( pixel_position );
...
- **Evolução do `draw( pixel::point )`:** O método torna-se um despachante semântico que identifica a célula e o sub-pixel ( up/down ) correspondente, delegando a complexidade de índice para a `surface_view`.
