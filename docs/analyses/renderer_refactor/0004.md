...
O domínio TUI deve ser explicitamente dividido em namespaces para evitar prefixos redundantes:
- **`pixel`**: Namespace contendo `point` ( sub-célula ), `line` e `rectangle`.
- **`cell`**: Namespace contendo `point` ( coordenada de célula no terminal ).
As ferramentas de tradução ( `to_cell`, `is_upper` ) devem ser puras e sem efeitos colaterais.

### 3. Alinhamento de Maquinaria
O terminal deixa de operar em coordenadas genéricas e passa a exigir `cell::point`, garantindo que apenas dados já traduzidos pelo renderizador cheguem à camada de exibição física.
