...
This project is guided by the principle of **Visual Serenity**. The goal is to lift code from being a set of imperative instructions for the CPU ("how") to a declarative description of domain intent ("what").

## 1. The Essence of Intent-Driven Design
...
- **Implementation vs. Intent:** Pointers, iterators, and manual loops are *low-level details*. References, objects, and ranges are *abstractions*. Visual Serenity means hiding these details to let the code's intent speak.
...
Clarity requires the elimination of visual noise. The syntax of C++ should not distract from the meaning.
...
- **Reference over Indirection:** Prefer `const&` over `->`. A pointer implies an address in memory; a reference implies the object itself.
...
- **Lambdas as a Last Resort:** If you are writing a lambda, you are likely failing to abstract the logic. Ask: "Should this be a reusable concept in `sak`?"
- **The "Sak" Bridge:** The `sak` (Swiss Army Knife) library is the project's dictionary. It translates low-level C++ mechanics into high-level tools.

## 4. Examples of Abstraction (Directional Vector)

These are not strict rules to follow, but examples of the **serene path**:

### A. Geometrical Relationship
Instead of checking `x1 <= x2` and `y1 <= y2` (technical), we declare a relationship:
```cpp
// Serene: What is the relationship?
...
```cpp
// Serene: What is the formula?
return sqrt( sum( m_data | square ) );
```

### C. Domain Movement
Instead of manually updating `x += 1` based on an enum (imperative), we apply the direction's inherent property:
```cpp
// Serene: How does the domain behave?
position += use_direction( m_direction ).value;
```

### D. Pattern Retrieval
Instead of an `if( find != end )` check (technical), we express the intent of a fallback:
```cpp
// Serene: What is the desired outcome?
...
2. **Abstract the Syntax:** Can I remove `::` or `->` to make it cleaner?
