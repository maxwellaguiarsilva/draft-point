...
##  2. **Naming Convention:**
    ...
    *   Template parameters must be prefixed with `t_` (e.g., `t_type`, `t_iterator`).
...
##  9. **`using` Clauses and `__using` Macro:**
    *   Use `using` clauses or the `__using` macro to avoid fully qualifying type names and functions.
    *   `using namespace` is strictly forbidden.
    *   Prefer the `__using` macro from `sak/using.hpp` when importing multiple symbols from the same prefix (namespace or class). This macro supports up to 64 parameters.
    *   `using` clauses must always start from the global scope using `::`.
    *   `using` clauses and the `__using` macro must be indented with a single tab.
    *   **Example (Single):**
        ```cpp
        using	::std::chrono::high_resolution_clock;
        ```
    *   **Example (Multiple with `__using`):**
        ```cpp
        __using( ::std::
            ,array
            ,vector
