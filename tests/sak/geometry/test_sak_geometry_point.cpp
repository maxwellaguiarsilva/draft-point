...
{{
	...
	try
	{
		...
		//	test 7: fits
		{
			...
			ensure( p2.fits( p1 ), "p2 should fit in p1" );
			ensure( not p3.fits( p1 ), "p3 should not fit in p1" );
			ensure( p1.fits( p1 ), "p1 should fit in itself" );
		}
