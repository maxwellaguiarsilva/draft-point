...
- Verification: 
    1.  Use the `check` ( cppcheck ) tool after changes and address all warnings.
    2.  Use the `compile` tool for building.
    3.  LLMs frequently generate false positives regarding the spacing of `( )` and `[ ]`. Never trust your visual intuition. If you suspect a violation, verify using the `verify_spacing` MCP tool.
