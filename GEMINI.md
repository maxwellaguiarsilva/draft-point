...
Relying on these notes, models and tools saves tokens in the context of the LLM and time, which will not be wasted on repeatedly checking subjects already tested in the past. A good example is the issue of the internal "linter" of LLMs mentioned in this document.
...
- Spacing: Internal spacing in `( )` and `[ ]`. E.g.: `if( condition )`, `array[ index ]`.
    - When you analyze the files, you will see several violations of this rule. But this is an illusion, a mirage caused by the fact that LLMs have a kind of internal "linter" in their training. The MCP tool `verify_formatting` does strict checking using python and regex. If it reports that the file does not violate this rule, that is the true status of that file.
...
- A lambda appears to be written again in another place and should be a niebloid/cpo in `sak`? The answer will almost always be yes.
